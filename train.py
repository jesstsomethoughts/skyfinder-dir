...
from loss import *
from dataloader import SkyFinderDataset
from utils import *
...
# Specify device
parser.add_argument('--device', type=str, default='cpu', help='Which device to use (i.e. cpu, mps, gpu)') # adding this option so it's Mac compatible
# LDS
...
parser.add_argument('--dataset', type=str, default='skyfinder', choices=['skyfinder'], help='dataset name')
...
# Start running model on TRAINING dataset split
def train(train_loader, model, optimizer, epoch, device):
    ...
    for idx, (inputs, targets, weights) in enumerate(train_loader):
        ...
        inputs, targets, weights = \
            inputs.to(device,non_blocking=True), targets.to(device,non_blocking=True), weights.to(device,non_blocking=True)
    ...
    if args.fds and epoch >= args.start_update:
        ...
        encodings, labels = torch.from_numpy(np.vstack(encodings)).to(device), torch.from_numpy(np.hstack(labels)).to(device)
...
def validate(device, val_loader, model, train_labels=None, prefix='Val'):
...
# Calculate metrics by shot ('many', 'median', and 'low')
...
def main():
    ...
    train_labels = df_train['daylight']

    train_dataset = SkyFinderDataset(data_dir=args.data_dir, df=df_train, img_size=args.img_size, split='train',
                          reweight=args.reweight, lds=args.lds, lds_kernel=args.lds_kernel, lds_ks=args.lds_ks, lds_sigma=args.lds_sigma)
    val_dataset = SkyFinderDataset(data_dir=args.data_dir, df=df_val, img_size=args.img_size, split='val')
    test_dataset = SkyFinderDataset(data_dir=args.data_dir, df=df_test, img_size=args.img_size, split='test')
    ...
    if args.evaluate:
        ...
        validate(device, test_loader, model, train_labels=train_labels, prefix='Test')
        return
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            checkpoint = torch.load(args.resume) if args.device != 'gpu' else \
                torch.load(args.resume, map_location=torch.device(f'cuda:{str(args.device)}'))
    ...
    for epoch in range(args.start_epoch, args.epoch):
        ...
        val_loss_mse, val_loss_l1, val_loss_gmean = validate(device, val_loader, model, train_labels=train_labels)
    ...
    test_loss_mse, test_loss_l1, test_loss_gmean = validate(device, test_loader, model, train_labels=train_labels, prefix='Test')
